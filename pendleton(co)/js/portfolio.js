...
function renderProperties() {
  ...
  grid.innerHTML = filteredProperties.map(p => `
    <article class="card card--property reveal">
      ${p.image ? `<img class="card__image" src="${p.image}" alt="${p.name}">` : `<div style="height:170px;background:linear-gradient(135deg,#1B2B3A 0%,#2C4A63 100%);display:flex;align-items:center;justify-content:center;"><span style="font-family:var(--font-display);font-size:3rem;color:rgba(75,172,198,0.3)">${p.name.charAt(0).toUpperCase()}</span></div>`}
      <div class="card__body">
        <div class="card__label">${p.type}</div>
        <h3 class="card__title">${p.name}</h3>
        <p class="card__text" style="font-size:0.875rem;color:var(--clr-text-muted);">${p.location}</p>
        <div class="card__meta">
          <span class="card__badge">${p.projectType}</span>
          ${p.units ? `<span class="card__badge">${p.units} Units</span>` : ''}
          ${p.sqft ? `<span class="card__badge">${(p.sqft/1000).toFixed(0)}K SF</span>` : ''}
        </div>
      </div>
      <div class="card__footer">
        <span style="font-family:var(--font-display);font-weight:700;color:var(--clr-accent);">${p.valueLabel}</span>
        <a href="../../pages/portfolio/property.html?slug=${encodeURIComponent(p.slug)}" class="btn btn--ghost btn--sm">View Details</a>
      </div>
    </article>
  `).join('');
